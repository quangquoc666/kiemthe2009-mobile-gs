...
local USED_LIMIT = 5
-- ************************** --
local USED_LIMIT_TYPE ={5, 10, 15,20}
-- ****************************************************** --
...
function HoatKhiTan:OnUse(scene, item, player, otherParams)
	...
	if totalUsedToday >= USED_LIMIT_TYPE[nLevel] then
		player:AddNotification("Mỗi ngày chỉ có thể sử dụng tối đa " .. USED_LIMIT_TYPE[nLevel] .. " bình " ..item:GetName() .. "!")
		return
	end
	...
	player:AddNotification("Sử dụng thành công, gia tăng hoạt lực " .. addPoint .. " điểm, ngày hôm nay đã dùng tổng cộng " .. totalUsedToday .. "/" .. USED_LIMIT_TYPE[nLevel] .. " " ..item:GetName() .. ".")
	-- ************************** --
